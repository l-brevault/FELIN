...
def Command_stage_1(t,m,N_eng,Pa,Exit_nozzle_area,Pitch_over_duration,\
    ...
    if t< Delta_vertical_phase :        
    ...
    elif ((t>=Delta_vertical_phase) and (t<Delta_vertical_phase + Pitch_over_duration)):    ###basculement
        
        theta = (gamma*180./np.pi - Delta_theta_pitch_over * (t - Delta_vertical_phase) / Pitch_over_duration)*np.pi/180.
        alpha = theta- gamma
        
    elif ((t>=Delta_vertical_phase + Pitch_over_duration) and (t<Delta_vertical_phase + Pitch_over_duration+ 3*Duration_decay)):    ###basculement
        ...
        if NX > Spec_NX:
            ...
            if debit_NX>Cst.derating_stage_1*mass_flow_rate_0:
            
                mass_flow_rate = debit_NX*N_eng
